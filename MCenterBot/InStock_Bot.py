...
linkTar = "https://www.microcenter.com/product/641408/lenovo-legion-5-156-gaming-laptop-computer-blue"
...
itemName =sDriver.find_element_by_css_selector('#product-details-control #details h1').text

try:
    #see if the store selector is present, if it is choose the first item which is the ca- Tustin store.
    # and click that selection.
    elem = WebDriverWait(sDriver, 10).until(
        EC.presence_of_element_located((By.ID,'storeInfo'))
        
    )
    tustin = sDriver.find_element_by_class_name('dropdown-item').click()

finally:  
    el = sDriver.find_element_by_class_name('inventory')
    ...
    if(searchString.find('SOLD OUT') != -1):
        ...
        emailNotif.sendNotif(itemName+' Is Still Out Of Stock')
    else:

        numStock = sDriver.find_element_by_class_name('inventoryCnt').text
        ...
        # emailNotif.sendNotif(bodyMessage) this will cause an email to be sent to the user instead of an SMS.
